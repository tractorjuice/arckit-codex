 ...
 *   rawRe:      pattern that matches a raw "/arckit:cmd ..." or "$arckit-cmd ..." prompt
...
const RECIPES = [
  {
    command: 'search',
    rawRe: /^\s*(?:\/arckit[.:]+|\$arckit-)search\b/i,
  ...
  {
    command: 'impact',
    rawRe: /^\s*(?:\/arckit[.:]+|\$arckit-)impact\b/i,
  ...
  },
  {
    ...
    rawRe: /^\s*(?:\/arckit[.:]+|\$arckit-)graph-report\b/i,
  ...
  {
    command: 'navigator',
    rawRe: /^\s*(?:\/arckit[.:]+|\$arckit-)navigator\b/i,
  ...
  {
    command: 'analyze',
    rawRe: /^\s*(?:\/arckit[.:]+|\$arckit-)analyze\b/i,
  ...
  {
    command: 'health',
    rawRe: /^\s*(?:\/arckit[.:]+|\$arckit-)health\b/i,
  ...
  },
  {
    ...
    rawRe: /^\s*(?:\/arckit[.:]+|\$arckit-)traceability\b/i,
...
function parseProjectArg(prompt, command) {
  const text = prompt.replace(new RegExp(`^(?:/arckit[.:]+|\\$arckit-)${command}\\s*`, 'i'), '');
...
function formatSearch(graph, prompt) {
  const text = prompt.replace(/^(?:\/arckit[.:]+|\$arckit-)search\s*/i, '').trim();
...
function formatImpact(graph, prompt) {
  const text = prompt.replace(/^(?:\/arckit[.:]+|\$arckit-)impact\s*/i, '').trim();
...
function formatHealth(graph, prompt, repoRoot) {
  const text = prompt.replace(/^(?:\/arckit[.:]+|\$arckit-)health\s*/i, '');
...
export function runGraphInjectHook(data = {}) {
...
function main() {
  const output = runGraphInjectHook(parseHookInput());
