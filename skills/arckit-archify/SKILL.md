...
```bash
node .arckit/scripts/archify-detect.mjs --json
```
...
source, follow `.arckit/references/citation-instructions.md` and
...
2. Render:
   ...
   ```bash
   node .arckit/scripts/owm-to-html.mjs /tmp/arckit-wardley-render.owm \
...
Use `.arckit/scripts/generate-document-id.mjs --next-num` for the
...
```bash
cat .arckit/templates/architecture-diagram-template.md
```
